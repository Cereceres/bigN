...
class bigN {
  /**
   * @class contructor
   * @param {string} the number to be instances
   * @param {number} the decimal to be used
   *
   */
  ...
  /**
   * sum method
   * @param {string} number to be added
   */
  add (number) {
    //split the number in decimal a integer part
    ...
    // set the auxiliary number
    let a, b
    // the complentary part
    let _res = 0,
      res = [ ]
    // the decimal numbers to be used  
    let numDecimal = decimal.length > this.decimal ? decimal.length : this.decimal
    // maz number of decimals
    ...
    // get the number like array
    this.number = res
    // set the zeros found them
  ...
  /**
   * multiply method
   * @param {string} number to be multplyed
   *
   */
  multiply (number) {
    let decimal = [ ]
  ...
  /**
   * rest method
   * @param {string} param to ve rested
   *
   */
  rest (number) {
    let num = number.toString().split('.')
